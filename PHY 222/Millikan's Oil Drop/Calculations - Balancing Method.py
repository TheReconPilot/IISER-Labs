...
# Load Data
# Droplet No., Fall Time, Balancing Voltage
data = np.loadtxt("Data - Balancing Method.csv", delimiter=",", skiprows=1)
...
## Here, fall_time and V are 2D arrays
## We do not want the 0 values in the data of V
for i in range(number_of_droplets):
    fall_time.append(unp.uarray([data[j,1] for j in range(len(data)) if data[j,0]==i+1], 0.1))
    V.append(unp.uarray([data[j,2] for j in range(len(data)) if data[j,0]==i+1 and data[j,2] != 0], 0.5))
...
# Calculate Mean Fall Time and Final Balancing Voltage
tf = [np.mean(i) for i in fall_time]
Vb = [np.mean(i) for i in V]
...
ne = [C * r3[i] / Vb[i] for i in range(len(Vb))]
